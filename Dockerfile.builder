...
RUN mkdir -p occt/build && cd occt/build \
    && emcmake cmake .. \
        ...
        "-DCMAKE_C_FLAGS=-fwasm-exceptions -O3 -msimd128 -ffp-contract=off -DIGNORE_NO_ATOMICS=1 -DOCCT_NO_PLUGINS" \
        "-DCMAKE_CXX_FLAGS=-fwasm-exceptions -O3 -msimd128 -ffp-contract=off -DIGNORE_NO_ATOMICS=1 -DOCCT_NO_PLUGINS" \
        -Wno-dev \
