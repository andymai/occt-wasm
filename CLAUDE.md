...
Any facade/codegen change invalidates the committed `wasm.br`. CI's "WASI build + stale-check" rebuilds it and **fails** if the bytes differ, uploading a fresh `occt-wasm.wasm.br-fresh` artifact. Run `scripts/reembed-wasmbr.sh <pr>` to refresh it: it checks out the PR, finds that commit's Build WASI run, downloads and validates the artifact, and commits it (add `--push` to push to the branch). A local rebuild is **not** byte-identical to CI, so re-embedding always takes CI's artifact, never a local `build-wasi`. To do it by hand: `gh run download <id> -n occt-wasm.wasm.br-fresh`, drop it at `crate/src/occt-wasm.wasm.br`, and commit. Bumping the OCCT submodule additionally requires rebuilding + pushing the builder image (CI links against its baked libs).
...
- release-please versions **only** the npm package; `crate` and `Cargo.lock` are in its `exclude-paths`, so a Rust-only change cuts no npm release. The crate's own version is bumped by hand in a `chore(crate): release X.Y.Z` PR, then tagged. A commit that touches `xtask/` as well still bumps npm, since codegen changes can reach the facade.
